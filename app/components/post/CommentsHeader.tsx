        ...
        <>
            <div className="flex items-center justify-between px-8">
                <div className="flex items-center">
                    ...
                    <div className="ml-3 pt-0.5">
                        ...
                        <Link 
                            ...
                            className="relative z-10 text-[17px] text-light-orange font-semibold hover:underline"
                        >
                            {post?.profile.name}
                        </Link>

                        <div className="relative z-0 text-[13px] text-zinc -mt-5 font-light">
                            {post?.profile.name}
                ...
                {contextUser?.user?.id == post?.user_id ? (
                    <div>
                        {isDeleteing ? (
                            <BiLoaderCircle className="animate-spin text-zinc" size="25"/>
                        ) : (
                            <button disabled={isDeleteing} onClick={() => deletePost()}>
                                <BsTrash3 className="cursor-pointer text-zinc" size="25"/>
                            </button>
                        )}
                    </div>
                ) : null}
            ...
            <p className="px-8 mt-4 text-zinc text-sm">{post?.text}</p>

            <p className="flex item-center text-zinc gap-2 px-8 mt-4 text-sm font-bold">
                <ImMusic size="17"/>
            ...
            <div className="flex items-center px-8 mt-8">
                <ClientOnly>
                    <div className="pb-4 text-center flex items-center">
                        ...
                        <span className="text-xs pl-2 pr-4 text-zinc font-semibold">
                            {likesByPost.length}
                        </span>
                    </div>
                </ClientOnly>
                ...
                <div className="pb-4 text-center flex items-center">
                    ...
                    <span className="text-xs pl-2 text-zinc font-semibold">{commentsByPost?.length}</span>
                </div>
            </div>
        </>
    )
